...
def create_train_data():
    train_data_path = os.path.join(data_path, 'Train_Data') # было, работает с оригинальным датасетом
    #train_data_path = os.path.join('Data\\new_train_data_test\\BCCD_Dataset-master\\BCCD\\JPEGImages')  # было, работает с оригинальным датасетом
    ...
    imgs = np.ndarray((int(total), image_rows, image_cols, 3), dtype=np.uint8)
...
def load_test_data():
    ...
    #imgs_id = np.load('imgs_id_test.npy') # было
    imgs_size = np.load('imgs_size.npy',  allow_pickle=True)
